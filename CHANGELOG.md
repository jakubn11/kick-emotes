...
- **Enter inserts the highlighted autocomplete suggestion.** After navigating the popup with ↑/↓, Enter now completes the focused emote instead of sending the half-typed message. With nothing explicitly focused, Enter still sends as before — Tab remains the "just take the top match" key. The popup's hint line now reads `Tab/Enter select` to match.
